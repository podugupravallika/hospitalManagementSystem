...
def patient_signup_view(request):
    userForm=forms.PatientUserForm_()
    patientForm=forms.PatientForm_()
    mydict={'userForm':userForm,'patientForm':patientForm}
    if request.method=='POST':
        userForm=forms.PatientUserForm_(request.POST)
        patientForm=forms.PatientForm_(request.POST,request.FILES)
...
def patient_dashboard_view(request):
    patient= models.Patientt.objects.get(user_id=request.user.id)
    dict={

...
def afterlogin_view(request):
    ...
    return HttpResponseRedirect("Unauthorized", status=401)
...
def nurse_signup_view(request):
    ...
    doctorForm=forms.NurseForm()
    mydict={'userForm':userForm,'doctorForm':doctorForm}
    if request.method=='POST':
        userForm=forms.NurseUserForm(request.POST)
        doctorForm=forms.NurseForm(request.POST,request.FILES)
        if userForm.is_valid() and doctorForm.is_valid():
            user=userForm.save()
            user.set_password(user.password)
            user.save()
...
def nurse_view_patient_view(request):
    ...
    return render(request,'hospital/nurse_view_patient.html',{'patients':patients,'doctor':nurse})
...
def nurse_view_discharge_patient_view(request):
    dischargedpatients=models.PatientDischargeDetails.objects.all().distinct().filter(assignedDoctorName=request.user.first_name)
    nurse=models.Nurse.objects.get(user_id=request.user.id) #for profile picture of nurse in sidebar
    return render(request,'hospital/nurse_view_discharge_patient.html',{'dischargedpatients':dischargedpatients,'doctor':nurse})
...
def nurse_appointment_view(request):
    nurse=models.Nurse.objects.get(user_id=request.user.id) #for profile picture of nurse in sidebar
    return render(request,'hospital/nurse_appointment.html',{'doctor':nurse})
...
def nurse_view_appointment_view(request):
    ...
    patients=models.Patient.objects.all().filter(status=True,user_id__in=patientid)
    appointments=zip(appointments,patients)
    return render(request,'hospital/nurse_view_appointment.html',{'appointments':appointments,'doctor':nurse})
...
def nurse_delete_appointment_view(request):
    ...
    patients=models.Patient.objects.all().filter(status=True,user_id__in=patientid)
    appointments=zip(appointments,patients)
    return render(request,'hospital/nurse_delete_appointment.html',{'appointments':appointments,'doctor':nurse})



@login_required(login_url='nurselogin')
@user_passes_test(is_nurse)
def delete_appointment_view_(request,pk):
    appointment=models.Appointment.objects.get(id=pk)
    appointment.delete()
    nurse=models.Nurse.objects.get(user_id=request.user.id) #for profile picture of nurse in sidebar
    appointments=models.Appointment.objects.all().filter(status=True,doctorId=request.user.id)
...
@login_required(login_url='nurselogin')
@user_passes_test(is_nurse)
def nurse_book_appointment_view(request):
    appointmentForm=forms.PatientAppointmentForm()
    patient=models.Patient.objects.get(user_id=request.user.id) #for profile picture of patient in sidebar
    message=None
    mydict={'appointmentForm':appointmentForm,'patient':patient,'message':message}
    if request.method=='POST':
        ...
        if appointmentForm.is_valid():
            ...
            doctor=models.Nurse.objects.get(user_id=request.POST.get('doctorId'))
    ...
    return render(request,'hospital/nurse_book_appointment.html',context=mydict)
